...
export const MemoizedBrokerModal = React.memo(function BrokerModal({
  ...
  const renderBrokerLinkButton = (label: string, onClick: () => void) => (
    <button
      type="button"
      className="gp-broker-link"
      onClick={onClick}
  ...
  const handleOrderSubmit = (e: React.FormEvent) => {
    ...
    const parsedPrice = orderType === "market"
      ? (Number.isFinite(orderIntent?.triggerPrice) && (orderIntent?.triggerPrice ?? 0) > 0 ? orderIntent?.triggerPrice ?? null : null)
      : parsePositiveFiniteNumber(price);
    const parsedTakeProfitPrice = takeProfit ? parsePositiveFiniteNumber(takeProfitPrice) : null;
    const parsedStopLossPrice = stopLoss ? parsePositiveFiniteNumber(stopLossPrice) : null;

    if (parsedQty === null || parsedPrice === null || (takeProfit && parsedTakeProfitPrice === null) || (stopLoss && parsedStopLossPrice === null)) {
      addNotification({
        title: "Ordre papier invalide",
        message: "Renseignez une quantité, un prix et des niveaux TP/SL strictement positifs.",
        type: "error",
    ...
    const newOrder: Order = {
      ...
      ...(takeProfit && parsedTakeProfitPrice !== null ? { takeProfitPrice: parsedTakeProfitPrice } : {}),
      ...(stopLoss && parsedStopLossPrice !== null ? { stopLossPrice: parsedStopLossPrice } : {}),
    };

    dispatch(addOrder(newOrder));

    addNotification({
      title: "Ordre Papier Transmis",
      message: `Votre ordre papier de ${side.toUpperCase()} ${orderType.toUpperCase()} de ${parsedQty} Qty @ ${orderType === "market" ? "Marché" : parsedPrice + " FCFA"} a été placé avec succès.`,
      type: "success",
  ...
  const renderConnectedBrokerFlow = () => {
    ...
    const effectivePrice = orderType === "market" ? (orderIntent?.triggerPrice ?? 0) : Number(price);
    const effectiveQty = Number(qty);
    const totalEst = Number.isFinite(effectivePrice) && effectivePrice > 0 && Number.isFinite(effectiveQty) && effectiveQty > 0 ? effectivePrice * effectiveQty : 0;
    ...
    <div className="gp-broker-overlay" onClick={resetModal}>
      ...
      >
        ...
        {!selectedBroker ? (
          ...
          <div className="gp-broker-connect-body">
            ...
            <div className="gp-broker-connect-header">
              ...
              <div style={{ textAlign: "left" }}>
                ...
                <div className="d-flex align-items-center gap-3 mt-1">
                  ...
                  {renderBrokerLinkButton("Learn More", () => showBrokerInfo("Courtier papier", selectedBroker.name + " est présenté dans un flux simulé; aucune connexion broker réelle n'est effectuée."))}
                </div>
              </div>
            </div>
            ...
            <div className="gp-broker-disclaimer">
              By clicking &quot;{orderIntent ? "Connect & Review Order" : "Connect"}&quot; I confirm that this is a simulated paper-trading flow and accept all risks.
            </div>
        ...
        <div className="gp-broker-footer">
          {!selectedBroker ? (
            <>
              Need a broker? {renderBrokerLinkButton("Let us know.", () => showBrokerInfo("Demande de courtier", "La collecte des demandes broker sera branchée dans une phase produit séparée."))}
            </>
          ) : (
            <>
              <button
              ...
              <span className="mx-2 text-secondary">|</span> Cannot connect to broker? <br /> {renderBrokerLinkButton("Let us know.", () => showBrokerInfo("Connexion broker", "Ce flux est simulé; aucune connexion externe n'est tentée dans cette version."))}
            </>
          )}
        </div>
      </div>

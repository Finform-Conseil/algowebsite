...
/**
 * Custom color picker for Technical Analysis drawing controls.
 * Supports HSVA state, HEX output, alpha changes, and preset colors.
 */
export const ProColorPicker: React.FC<ProColorPickerProps> = ({
  ...
  useEffect(() => {
    ...
    setHsva((currentHsva) => {
      const isSame =
        Math.abs(currentHsva.h - nextHsva.h) < 0.1 &&
        Math.abs(currentHsva.s - nextHsva.s) < 0.1 &&
        Math.abs(currentHsva.v - nextHsva.v) < 0.1 &&
        Math.abs(currentHsva.a - nextHsva.a) < 0.01;

      return isSame ? currentHsva : nextHsva;
    });
  }, [color, opacity, prevProps.color, prevProps.opacity]);

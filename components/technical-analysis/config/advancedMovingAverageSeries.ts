...
export const buildAdvancedMovingAverageSeriesDefinitions = (
  ...
  const activeByFamily: Record<AdvancedMovingAverageFamily, Set<number>> = {
    wma: new Set(normalizeMovingAveragePeriods(state.activeWma)),
    dema: new Set(normalizeMovingAveragePeriods(state.activeDema)),
    tema: new Set(normalizeMovingAveragePeriods(state.activeTema)),
    hma: new Set(normalizeMovingAveragePeriods(state.activeHma)),
    zlema: new Set(normalizeMovingAveragePeriods(state.activeZlema)),
    alma: new Set(normalizeMovingAveragePeriods(state.activeAlma)),
    smma: new Set(normalizeMovingAveragePeriods(state.activeSmma)),
    kama: new Set(normalizeMovingAveragePeriods(state.activeKama)),
    vwma: new Set(normalizeMovingAveragePeriods(state.activeVwma)),
  };
...
export const isAdvancedMovingAverageActive = (
  ...
  const activePeriods = spec.family === "wma"
    ? state.activeWma
    : spec.family === "dema"
      ? state.activeDema
      : spec.family === "tema"
...
export const toggleAdvancedMovingAverage = (
  ...
  if (spec.family === "wma") return { ...state, activeWma: nextPeriods };
  if (spec.family === "dema") return { ...state, activeDema: nextPeriods };
  if (spec.family === "tema") return { ...state, activeTema: nextPeriods };
  if (spec.family === "hma") return { ...state, activeHma: nextPeriods };
  if (spec.family === "zlema") return { ...state, activeZlema: nextPeriods };
  if (spec.family === "alma") return { ...state, activeAlma: nextPeriods };
  if (spec.family === "smma") return { ...state, activeSmma: nextPeriods };
  if (spec.family === "kama") return { ...state, activeKama: nextPeriods };
  return { ...state, activeVwma: nextPeriods };
};

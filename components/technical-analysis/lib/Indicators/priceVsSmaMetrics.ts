...
export interface PriceVsSmaMetricResult {
  spec: PriceVsSmaMetricSpec;
  state: PriceVsSmaMetricState;
  close: number | null;
  sma: number | null;
  ...
  qualityLabel: string;
  qualityTone: PriceVsSmaQualityTone;
  reason: string;
}
...
const resolveLiquidityQuality = (
  ...
  isConfirmedBar: boolean,
  timeframe: string,
): { label: string; tone: PriceVsSmaQualityTone } => {
...
export const calculatePriceVsSmaMetrics = (
  chartData: ChartDataPoint[],
  ...
  return PRICE_VS_SMA_METRIC_SPECS.map((spec) => {
    const sma = calculateSmaExact(closes, spec.period);
    const distance = close !== null && sma !== null ? close - sma : null;
    const distancePercent = distance !== null && sma !== null && sma !== 0 ? (distance / sma) * 100 : null;
    const epsilonPercent = tickSize !== null && sma !== null && sma !== 0 ? (tickSize / sma) * 100 : null;
    ...
    if (close === null) {
      return {
        ...
        qualityTone: "danger",
        reason: "Aucun close exploitable pour calculer la distance prix/SMA.",
      };
    }

    if (sma === null || distance === null || distancePercent === null || epsilonPercent === null) {
      return {
        spec,
        state: "unknown",
        close,
    ...
    return {
      ...
      reason: `${spec.label} = ((Close - ${spec.shortLabel}) / ${spec.shortLabel}) × 100 sur la timeframe ${timeframe}.`,
    };
  });
};

...
const getPriceAxisInfo = (chart: EChartsInstance): { yAxisIdx: number; gridIdx: number } => {
  try {
    ...
    return { yAxisIdx: candlestick?.yAxisIndex ?? 0, gridIdx: candlestick?.gridIndex ?? 0 };
  } catch {
...
const calculateVolumeProfileHDR = (
  ...
  rangeData.forEach(bar => {
    ...
    if (lowIdx === highIdx) {
      ...
      for (let i = lowIdx; i <= highIdx; i++) {
        ...
        if (isUp) bins[idx].upVolume += volPerBin;
        else bins[idx].downVolume += volPerBin;
  ...
  while (currentArea < targetArea && (upIdx < numRows - 1 || downIdx > 0)) {
    ...
    if (upNext >= downNext && upIdx < numRows - 1) {
      upIdx++;
      currentArea += upNext;
    } else if (downIdx > 0) {
      downIdx--;
      currentArea += downNext;
    } else break;
  }
...
export const renderFixedRangeVolumeProfile = (
  ...
  ctx.moveTo(pts[0].x, 0);
  ctx.lineTo(pts[0].x, ctx.canvas.height);
  ctx.moveTo(pts[1].x, 0);
  ctx.lineTo(pts[1].x, ctx.canvas.height);
  ctx.stroke();
  ctx.restore();

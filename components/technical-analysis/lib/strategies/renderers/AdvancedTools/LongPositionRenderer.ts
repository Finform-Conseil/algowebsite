...
export function renderLongPosition(
    ...
    if (isFillEnabled) {
        ...
        h.ctx.fillRect(x - width / 2, tpY, width, entry.y - tpY);
        h.ctx.strokeStyle = tpColor;
        h.ctx.globalAlpha = Math.min(1, tpOpacity * 2);
        h.ctx.lineWidth = 1;
        h.ctx.strokeRect(x - width / 2, tpY, width, entry.y - tpY);

        h.ctx.globalAlpha = slOpacity;
        h.ctx.fillStyle = slColor;
        h.ctx.fillRect(x - width / 2, entry.y, width, slY - entry.y);
        h.ctx.strokeStyle = slColor;
        h.ctx.globalAlpha = Math.min(1, slOpacity * 2);
        h.ctx.lineWidth = 1;
        h.ctx.strokeRect(x - width / 2, entry.y, width, slY - entry.y);
        h.ctx.restore();
    }
    h.ctx.save();
    ...
    renderPositionStats(x, entry.y, tpPrice, slPrice, entryPrice, drawing, true, h);
    renderPriceTag(x + width / 2, tpY, tpPrice, tpColor, "right", 12, drawing, h);
    renderPriceTag(x + width / 2, slY, slPrice, slColor, "right", 0, drawing, h);
...
export function hitTestLongPosition(
    mx: number,
    my: number,

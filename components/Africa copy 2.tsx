...
// Définition des bourses avec leurs données complètes
const stockExchanges = [
  {
    ...
    pinCoordinates: { x: 280, y: 260 },
    color: '#10B981',
    data: AFRICAN_EXCHANGES.find(e => e.id === 'brvm')
  },
  {
    id: 'NGX',
    ...
    shortName: 'NGX',
    countries: ['NG'],
    pinCoordinates: { x: 350, y: 350 },
    color: '#8B5CF6',
    data: AFRICAN_EXCHANGES.find(e => e.id === 'ngx')
  },
  {
    id: 'NSE',
    ...
    shortName: 'NSE',
    countries: ['KE'],
    pinCoordinates: { x: 730, y: 480 },
    color: '#F59E0B',
    data: AFRICAN_EXCHANGES.find(e => e.id === 'nse')
  },
  {
    id: 'JSE',
    ...
    shortName: 'JSE',
    countries: ['ZA'],
    pinCoordinates: { x: 570, y: 880 },
    color: '#EF4444',
    data: AFRICAN_EXCHANGES.find(e => e.id === 'jse')
  },
  {
    id: 'GSE',
    ...
    shortName: 'GSE',
    countries: ['GH'],
    pinCoordinates: { x: 250, y: 400 },
    color: '#06B6D4',
    data: AFRICAN_EXCHANGES.find(e => e.id === 'gse')
  },
  {
    id: 'CSE',
    ...
    shortName: 'CSE',
    countries: ['MA'],
    pinCoordinates: { x: 130, y: 120 },
    color: '#EC4899',
    data: AFRICAN_EXCHANGES.find(e => e.id === 'cse')
  },
];
...
const AfricaMap = () => {
  ...
  const getCountryColor = (countryId: string) => {
    const exchange = stockExchanges.find(ex => ex.countries.includes(countryId));
    if (!exchange) return 'var(--card-background)';
    // if (!exchange) return '#0f172a';
    const isActive = activeExchange === exchange.id;
    return isActive ? exchange.color : `${exchange.color}60`;
  };
  ...
  const handleBackToAfrica = () => {
    setSelectedExchange(null);
  };

  const RegionalComponent = selectedExchange ? exchangeComponents[selectedExchange as keyof typeof exchangeComponents] : null;
  const selectedExchangeData = stockExchanges.find(ex => ex.id === selectedExchange);
      ...
      <AnimatePresence mode="wait">
        {!selectedExchange ? (
          ...
          >
            ...
            {/* Votre SVG est directement intégré ici */}
            <svg
            ...
            {stockExchanges.map((exchange, index) => (
              <motion.div
                ...
                style={{
                  ...
                  top: `${(exchange.pinCoordinates.y / 1000) * 100}%`,
                  transform: 'translate(-50%, -50%)',
                  cursor: 'pointer',
                  zIndex: activeExchange === exchange.id ? 10 : 1
                }}
              >
                <motion.div
                  animate={{
                    scale: activeExchange === exchange.id ? 1.3 : 1,
                    y: activeExchange === exchange.id ? -5 : 0
                  }}
                ...
                >
                  ...
                  <div style={{
                    ...
                    {exchange.shortName.charAt(0)}
                  </div>
                </motion.div>

                <AnimatePresence>
                  {activeExchange === exchange.id && exchange.data && (
                    <motion.div
                    ...
                    >
                      <div style={{
                        ...
                        borderTop: `10px solid ${exchange.color}`
                      }} />
                      ...
                      <div style={{ display: 'grid', gap: '0.5rem', fontSize: '0.875rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: '#6B7280', fontWeight: '500' }}>Pays:</span>
                          <span style={{ color: '#1F2937', fontWeight: '600' }}>{exchange.data.country}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: '#6B7280', fontWeight: '500' }}>Devise:</span>
                          <span style={{ color: '#1F2937', fontWeight: '600' }}>{exchange.data.currency}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: '#6B7280', fontWeight: '500' }}>Actions listées:</span>
                          <span style={{ color: '#1F2937', fontWeight: '600' }}>{formatNumber(exchange.data.listedCompanies)}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: '#6B7280', fontWeight: '500' }}>Cap. totale:</span>
                          <span style={{ color: exchange.color, fontWeight: '700' }}>{formatCurrency(exchange.data.totalMarketCap)}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: '#6B7280', fontWeight: '500' }}>Cap. moyenne:</span>
                          <span style={{ color: exchange.color, fontWeight: '700' }}>{formatCurrency(exchange.data.averageMarketCap / 1000)}</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
          ...
          >
            ...
            <motion.button
              ...
              style={{
                ...
                background: selectedExchangeData?.color || 'var(--primary-color)',
                color: 'white',
                border: 'none',
                borderRadius: '0.5rem',
                fontSize: '0.875rem',
            ...
            <motion.div
              ...
              style={{
                ...
                border: `2px solid ${selectedExchangeData?.color}`,
                zIndex: 10
              }}
            >
              <div style={{
                fontSize: '0.75rem',
                fontWeight: '600',
                color: selectedExchangeData?.color,
                marginBottom: '0.25rem'
              }}>
            ...
            >
              {RegionalComponent && <RegionalComponent color={selectedExchangeData?.color || '#10B981'} />}
            </div>
          </motion.div>
        )}
      </AnimatePresence>

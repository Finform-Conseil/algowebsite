...
export default function PublicFinancesPage() {
  const { allSectorFinancesData, getAllSectorFinances, isMutationLoading } = useMacroRepository();
    ...
    if (comparisonMode === 'comparison' && selectedCountries.length > 0) {
      const loadComparisonData = async () => {
        ...
        for (const countryId of selectedCountries) {
          try {
            const result = await getAllSectorFinances({ 
  ...
  }, [comparisonMode, selectedCountries, getAllSectorFinances]);
  ...
  useEffect(() => {
    console.log("Sector Public Finances Data (raw):", allSectorFinancesData);
    console.log("Type:", typeof allSectorFinancesData);
    console.log("Keys:", allSectorFinancesData ? Object.keys(allSectorFinancesData) : 'undefined');
  }, [allSectorFinancesData]);

  const activeCountryData = useMemo(() => {
    if (!allSectorFinancesData) {
      console.log("No data available");
      return null;
    }
    
    console.log("Processing data:", allSectorFinancesData);
    return allSectorFinancesData as any;
  }, [allSectorFinancesData]);
      ...
      <div className="macro-header-wrapper">
        ...
        >
          <div className="header-content">
            <h1>Public Finance & Budget</h1>
            <p>Deficits, Debt, and Fiscal Policy</p>
          </div>

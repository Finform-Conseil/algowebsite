...
export default function FinancialSecondaryScreenerPage() {
  ...
  const filteredBonds = useMemo(() => {
    return mockBonds.filter(bond => {
      ...
      if (bond.yield < filters.yieldRange[0] || bond.yield > filters.yieldRange[1]) {
        return false;
      }
      if (bond.price < filters.priceRange[0] || bond.price > filters.priceRange[1]) {
        return false;
      }
  ...
  const insights = useMemo(() => {
    ...
    const avgYield = filteredBonds.reduce((sum, b) => sum + b.yield, 0) / filteredBonds.length;
    const avgExchangeRatio = filteredBonds.reduce((sum, b) => sum + b.exchangeRatio, 0) / filteredBonds.length;
    const totalTradedValue = filteredBonds.reduce((sum, b) => sum + b.tradedValue, 0);
  ...
  const categoriesRaw = (selectedCategories.length === 0 || selectedCategories.includes("All")
            ...
            <div className="filter-group">
              ...
              <div className="range-inputs">
                <input
                  ...
                  onChange={(e) => setFilters({ ...filters, yieldRange: [Number(e.target.value), filters.yieldRange[1]] })}
                  placeholder="Min"
                />
                <span>to</span>
                <input
                  ...
                  onChange={(e) => setFilters({ ...filters, yieldRange: [filters.yieldRange[0], Number(e.target.value)] })}
                  placeholder="Max"
                />
              </div>
            </div>
                  ...
                  {filteredBonds.map(bond => (
                    <tr key={bond.id} className={selectedBonds.includes(bond.id) ? 'selected' : ''}>
                      ...
                      <td className="isin-col">
                        <Link href={`/fixed-income/bond-detail/financial-secondary/${bond.isin}`} className="isin-link">
                          {bond.isin}
                        </Link>
                      </td>
                      ...
                      <td className="number-col">{bond.previousYield.toFixed(2)}</td>
                      <td className={`number-col ${bond.variation >= 0 ? 'positive' : 'negative'}`}>{bond.variation >= 0 ? '+' : ''}{bond.variation.toFixed(2)}</td>
                      <td className="number-col">{bond.price.toFixed(2)}</td>
                      <td className="number-col">{bond.previousPrice.toFixed(2)}</td>
                      <td className="number-col">{(bond.tradedVolume / 1000000).toFixed(1)}M</td>
                      <td className="number-col">{(bond.tradedValue / 1000000000).toFixed(2)}B</td>
                ...
                <tbody>
                  {filteredBonds.map(bond => (
                    <tr key={bond.id} className={selectedBonds.includes(bond.id) ? 'selected' : ''}>
                      ...
                      <td><span className="country-cell"><span className="country-flag">{getCountryFlag(bond.country)}</span>{bond.country}</span></td>
                      <td><span className="badge badge-bond-type">{bond.bondType}</span></td>
                      <td className="number-col">{(bond.issuedAmount / 1000000).toFixed(0)}M</td>
                      <td className="number-col">{(bond.mobilizedAmount / 1000000).toFixed(0)}M</td>
                      <td className="number-col">{(bond.outstanding / 1000000).toFixed(0)}M</td>
                      <td className="number-col">{bond.initialDuration.toFixed(1)}Y</td>
                      ...
                      <td><span className="badge badge-ost">{bond.esv}</span></td>
                ...
                <tbody>
                  {filteredBonds.map(bond => (
                    <tr key={bond.id} className={selectedBonds.includes(bond.id) ? 'selected' : ''}>
                      ...
                      <td><span className="country-cell"><span className="country-flag">{getCountryFlag(bond.country)}</span>{bond.country}</span></td>
                      <td><span className="badge badge-bond-type">{bond.bondType}</span></td>
                      <td>{new Date(bond.transactionDate).toLocaleDateString('en-US')}</td>
                      <td className="number-col">{bond.yield.toFixed(2)}</td>
                      <td className="number-col">{bond.previousYield.toFixed(2)}</td>
                      <td className="number-col">{(bond.tradedVolume / 1000000).toFixed(1)}M</td>
                      <td className="number-col">{(bond.tradedValue / 1000000000).toFixed(2)}B</td>
            ...
            <div className="summary-view">
              <div className="summary-stats-grid">
                <div className="stat-card">
                  ...
                  <div className="stat-value">{(filteredBonds.reduce((sum, b) => sum + b.tradedVolume, 0) / 1000000).toFixed(1)}M</div>
                  <div className="stat-label">Total Volume</div>
                  <div className="stat-change positive">+12.5% vs previous period</div>
                </div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-value positive">{filteredBonds.filter(b => b.variation > 0).length}</div>
                  <div className="stat-label">Positive Variation</div>
                  <div className="stat-change positive">+{((filteredBonds.filter(b => b.variation > 0).length / filteredBonds.length) * 100).toFixed(1)}%</div>
                </div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-value negative">{filteredBonds.filter(b => b.variation < 0).length}</div>
                  <div className="stat-label">Negative Variation</div>
                  <div className="stat-change negative">-{((filteredBonds.filter(b => b.variation < 0).length / filteredBonds.length) * 100).toFixed(1)}%</div>
                </div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-value">{filteredBonds.filter(b => b.variation === 0).length}</div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-value">{(filteredBonds.reduce((sum, b) => sum + b.yield, 0) / filteredBonds.length).toFixed(2)}%</div>
                  ...
                  <BarChart
                    data={{
                      ...
                      values: categoriesRaw.map(category => {
                        const bondsInCategory = filteredBonds.filter(b => b.bondCategory === category);
                        return bondsInCategory.reduce((sum, b) => sum + b.tradedVolume, 0) / 1000000;
                      }),
                    }}
                    height="350px"
                    color="#00BFFF"
                ...
                <div className="chart-card">
                  ...
                  <PieChart
                    data={categoriesRaw.map(category => ({
                      name: category,
                      value: filteredBonds.filter(b => b.bondCategory === category).reduce((sum, b) => sum + b.tradedVolume, 0) / 1000000
                    }))}
                    height="350px"
                  />
                </div>

...
export default function ExternalSectorPage() {
  const { allSectorForeignData, getAllSectorForeign, isMutationLoading } = useMacroRepository();
    ...
    if (comparisonMode === 'comparison' && selectedCountries.length > 0) {
      const loadComparisonData = async () => {
        ...
        for (const countryId of selectedCountries) {
          try {
            const result = await getAllSectorForeign({ 
  ...
  }, [comparisonMode, selectedCountries, getAllSectorForeign]);
  ...
  useEffect(() => {
    console.log("Sector External Data (raw):", allSectorForeignData);
    console.log("Type:", typeof allSectorForeignData);
    console.log("Keys:", allSectorForeignData ? Object.keys(allSectorForeignData) : 'undefined');
  }, [allSectorForeignData]);

  const activeCountryData = useMemo(() => {
    if (!allSectorForeignData) {
      console.log("No data available");
      return null;
    }
    
    console.log("Processing data:", allSectorForeignData);
    return allSectorForeignData as any;
  }, [allSectorForeignData]);
      ...
      <div className="macro-header-wrapper">
        ...
        >
          <div className="header-content">
            <h1>Foreign Trade & Currencies</h1>
            <p>International Trade and Exchange Rates</p>
          </div>

...
export default function KeyIndicatorsPage() {
  const { allSectorMonetaryData, getAllSectorMonetary, isMutationLoading } = useMacroRepository();
    ...
    if (comparisonMode === 'comparison' && selectedCountries.length > 0) {
      const loadComparisonData = async () => {
        ...
        for (const countryId of selectedCountries) {
          try {
            const result = await getAllSectorMonetary({ 
  ...
  }, [comparisonMode, selectedCountries, getAllSectorMonetary]);
  ...
  useEffect(() => {
    console.log("Sector Monetary Data (raw):", allSectorMonetaryData);
    console.log("Type:", typeof allSectorMonetaryData);
    console.log("Keys:", allSectorMonetaryData ? Object.keys(allSectorMonetaryData) : 'undefined');
  }, [allSectorMonetaryData]);

  const activeCountryData = useMemo(() => {
    if (!allSectorMonetaryData) {
      console.log("No data available");
      return null;
    }
    
    console.log("Processing data:", allSectorMonetaryData);
    return allSectorMonetaryData as any;
  }, [allSectorMonetaryData]);
      ...
      <div className="macro-header-wrapper">
        ...
        >
          <div className="header-content">
            <h1>Currency & Central Banks</h1>
            <p>Monetary policies and interest rates</p>
          </div>
